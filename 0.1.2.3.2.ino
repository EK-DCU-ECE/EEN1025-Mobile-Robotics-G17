...
// Timing (tune these)
int CLEAR_MARKER_MS = 220;   // longer helps avoid re-trigger on thick bars
int SPIN_90_MS      = 320;   // increased a bit to reduce "30 degree" turns
int SPIN_180_MS     = 640;   // about 2x SPIN_90_MS
...
// Turning guard (prevents marker logic interrupting turns)
...
void loop() {
  ...
  // If we are turning/reacquiring, do NOT run marker logic here
  if (isTurning) {
    delay(5);
    return;
  }
  ...
  // Release latch only after we are off marker for a bit
...
void handleMarkerHit() {
  ...
  // Save where we came from BEFORE updating indices
  ...
  if (isRealNode(curr)) {
    ...
    if (arrivedNode == targetNode) {
      ...
      // If next leg begins by going back to where we came from, do U-turn at node
...
void reacquireLineAfterTurn(bool keepTurningLeft, bool keepTurningRight) {
  ...
  // Move forward slightly to get off thick bar/junction blob
  setMotors(120, 120);
  delay(120);

  // Rotate slowly until center sees line AND not on full marker
  while (true) {
    readSensors();
  ...
  // Cooldown so we don't instantly re-trigger marker
