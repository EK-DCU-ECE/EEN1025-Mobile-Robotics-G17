...
// ===== SHARP IR DISTANCE (ADDED) =====
int readSharpCmFast(int analogPin);
...
// ===== SHARP IR DISTANCE SENSORS (ADDED) =====
// NOTE: On ESP32, prefer ADC1 pins when WiFi is enabled.
const int SHARP1_PIN = 35;   // set to your wiring (ADC1 recommended)
const int SHARP2_PIN = 36;   // set to your wiring (ADC1 recommended)
// Trigger thresholds (cm). Tune these for your specific Sharp models & mounting.
const int SHARP_STOP_CM = 10;
// How many of the 3 sensors must trigger to declare an obstacle (1 = any sensor).
const int OBSTACLE_REQUIRED_TRIGGERS = 1;
...
void setup() {
  ...
  // ESP32 ADC config for more stable readings.
...
void loop() {
  ...
  if (!onBar) {
    ...
    if (!returningToPrev && obstacleDetected()) {
...
// ===============================
// ===== OBSTACLE SENSOR FUSION (ADDED) =====
// ===============================
// Count how many of the available sensors are currently reporting an obstacle.
// This lets you choose:
//  - OBSTACLE_REQUIRED_TRIGGERS = 1  => any sensor triggers (widest FOV)
//  - OBSTACLE_REQUIRED_TRIGGERS = 2+ => require multiple sensors (more robust)
int obstacleTriggerCount() {
  bool us  = (usCm > 0 && usCm <= STOP_CM);
  bool s1  = (sharp1Cm > 0 && sharp1Cm <= SHARP_STOP_CM);
  bool s2  = (sharp2Cm > 0 && sharp2Cm <= SHARP_STOP_CM);
  return (us ? 1 : 0) + (s1 ? 1 : 0) + (s2 ? 1 : 0);
}

bool obstacleDetected() {
  return obstacleTriggerCount() >= OBSTACLE_REQUIRED_TRIGGERS;
}

// ===============================
// ===== SHARP IR DISTANCE (ADDED) =====
// ===============================
// Basic conversion for common Sharp analog IR distance sensors.
// This is an empirical power-law approximation. Tune SHARP_STOP_CM and/or the
// conversion if your model differs.
// Returns cm, or -1 if reading is invalid/out of range.
int readSharpCmFast(int analogPin) {
  // If not wired / disabled
  if (analogPin < 0) return -1;

  // Median-ish smoothing with a small sample average (fast & stable enough)
  const int N = 5;
  uint32_t acc = 0;
  for (int i = 0; i < N; i++) {
    acc += (uint32_t)analogRead(analogPin);
    delayMicroseconds(250);
  }
  float adc = (float)acc / (float)N;

  // Convert to volts (ESP32 default ADC is 12-bit: 0..4095)
  float v = adc * (3.3f / 4095.0f);
  if (v < 0.05f) return -1;

  // Typical GP2Y0A21-style curve: distance ≈ A * V^B
  // Works "okay" across short range; clamp to something sensible.
  float cm = 27.728f * powf(v, -1.2045f);

  if (!isfinite(cm) || cm < 2.0f || cm > 200.0f) return -1;
  return (int)(cm + 0.5f);
}
