...
// Marker detection window: all 5 sensors must see the line within this time window
const unsigned long MARKER_WINDOW_MS = 200;
...
// ===============================
// Node 5 two-hit rule + bypass (ported from node5_twohit_sharp3sensors)
// If targetNode == 5:
//   - 1st detection (<= NODE5_DETECT_CM) = obstacle -> bypass maneuver
//   - 2nd detection (<= NODE5_DETECT_CM) = terminal wall -> arriveAtWallNode5()
// Uses a cooldown + "clear gap" to avoid double-counting the same object.
// ===============================
const uint32_t NODE5_DETECT_COOLDOWN_MS = 700;
const int NODE5_DETECT_CM = 8;
const int NODE5_CLEAR_DETECT_CM = 30;

const uint32_t NODE5_BYPASS_FWD_MS = 600; // tune
const int NODE5_FWD_SPEED = 255;          // tune

int      node5DetectCount = 0;
bool     node5DetectArmed = true;
...
void setup() {
  ...
  if (targetNode == 5) { node5DetectCount = 0; node5DetectArmed = true; node5DetectCooldownUntil = 0; }
...
void loop() {
  ...
  if (!onBar) {
    ...
    // If we detect obstacle, trigger behaviour
    // NODE 5 two-hit special case (ported):
    //   - 1st detection => bypass
    //   - 2nd detection => arrive at wall node 5
    if (!returningToPrev && targetNode == 5 && detectCm > 0) {
      ...
      // Count a detection.
      if (node5DetectArmed && detectCm <= NODE5_DETECT_CM) {
        node5DetectArmed = false;
        node5DetectCooldownUntil = millis() + NODE5_DETECT_COOLDOWN_MS;
        node5DetectCount++;
        Serial.print("NODE5 detection #"); Serial.print(node5DetectCount);
      ...
      // else: keep going (line logic below)
    }

    // Generic obstacle: return to previous node (never blocks node-5 spur behaviour above)
    if (!returningToPrev && targetNode != 5 && detectCm > 0 && detectCm <= STOP_CM) {
      setMotors(0, 0);
      delay(60);
...
bool isMarker() {
  // A marker/node is detected when ALL 5 sensors have seen the line within a short time window.
  // This allows detection on corners where sensors may not go high at exactly the same instant.

  // IMPORTANT: while turning / re-acquiring, ignore marker detection and flush the window
  // so we don't "collect" partial hits and accidentally detect a second node.
  static unsigned long lastSeenMs[5] = {0, 0, 0, 0, 0};
  unsigned long now = millis();

  if (isTurning || now < ignoreMarkersUntil) {
    for (int i=0; i<5; i++) lastSeenMs[i] = 0;
    return false;
  }
  ...
  // If we're completely off the line right now, we're not on a marker.
  if (!anyOnLine) return false;

  for (int i=0; i<5; i++) {
    if (now - lastSeenMs[i] > MARKER_WINDOW_MS) return false;
  }
  return true;
}
...
// ===============================
// Node 5 spur bypass helpers (dead-reckoning turns: NO line reacquire)
// ===============================
void doLeft90Dead() {
  isTurning = true;
  setMotors(turnSpeed, -turnSpeed);
...
void doNode5BypassManeuver() {
  Serial.println("NODE5 BYPASS: obstacle detected -> left, forward, right, then continue to wall.");
  setMotors(0, 0);
  delay(80);
