...
void buildLeg(int fromNode, int toNode) {
  ...
  if (mlen == 0 || mp[mlen-1] != (int)fromM) {
    // If blocked segments made it disconnected, retry once with all blocks cleared.
    bool hadAny = false;
    for (int i=0; i<NUM_MARKERS; i++)
      for (int j=0; j<NUM_MARKERS; j++)
    ...
    // Fallback: direct (should be rare)
    legPath[0] = fromM;
    legPath[1] = toM;
    legLen = 2;
  } else {
