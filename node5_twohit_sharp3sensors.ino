...
const int STOP_CM  = 10;      // global obstacle stop at 5cm
const int CLEAR_CM = 12;      // resume at 8cm (hysteresis)
const unsigned long US_PERIOD_MS = 60;

// ===== SHARP IR (ADDED) =====
// NOTE: Set these to the ADC pins you actually wired the two Sharp sensors to.
const int SHARP1_PIN = 9;   // <-- change if needed
const int SHARP2_PIN = 10;   // <-- change if needed

int sharp1Cm = -1;
int sharp2Cm = -1;

int readSharpCmFast(int adcPin);
int minPositive3(int a, int b, int c);
...
// ===============================
// Node 5 special-case (MJ35 -> M5) dead-reckoning with obstacle bypass
// - There is no line on the spur. Line following is disabled 2s after leaving MJ35.
// - If we encounter an obstacle on the spur BEFORE the wall, we do a fixed bypass:
//   left -> forward -> right -> continue to wall.
// ===============================
const int NODE5_WALL_CM     = STOP_CM;  // treat <= this as arrival at node 5
const int NODE5_OBSTACLE_CM = 8;       // trigger bypass if <= this and > wall
const uint32_t NODE5_BYPASS_FWD_MS = 600; // sideways clearance distance (tune)
const int NODE5_FWD_SPEED   = 255;      // dead-reckoning forward speed toward wall


// Timing (tune these)
int SPIN_90_MS      = 420;
int CLEAR_FWD_SPEED = 255;
...
// ===== Node 5 spur mode flags =====
bool headingTo5Spur = false;     // true after leaving MJ35 toward M5 until wall arrival
bool node5BypassDone = false;    // ensure we only execute the bypass once per spur run
uint32_t node5SpurStartMs = 0;   // time we committed to the MJ35->M5 spur (for wall-vs-obstacle disambiguation)
// NEW RULE (USER): on the MJ35->M5 spur, the FIRST ultrasonic detection is the obstacle,
// the SECOND ultrasonic detection is the terminal wall.
// So we count detections instead of trying to disambiguate by distance/time.
int      node5DetectCount = 0;     // 0 = haven't seen anything yet, 1 = saw obstacle (bypass done), 2 = saw wall
bool     node5DetectArmed = true;  // re-arm after a "clear" gap
uint32_t node5DetectCooldownUntil = 0;
const uint32_t NODE5_DETECT_COOLDOWN_MS = 700; // prevent counting the same object twice
const int NODE5_DETECT_CM = 8;                // what counts as "we saw something" on the spur
const int NODE5_CLEAR_DETECT_CM = 30;          // must rise above this to re-arm

// Dead-reckoning turns (no line reacquire) for node-5 spur
void doLeft90Dead() {
  isTurning = true;
...
void doNode5BypassManeuver() {
  Serial.println("NODE5 BYPASS: obstacle on MJ35->M5 spur -> left, forward, right, then continue to wall.");
  setMotors(0, 0);
  delay(80);

  // We are on the spur: do NOT try to reacquire line.
  doLeft90Dead();
  setMotors(NODE5_FWD_SPEED, NODE5_FWD_SPEED);
  delay(NODE5_BYPASS_FWD_MS);
  setMotors(0, 0);
  delay(60);
  doRight90Dead();

  // Continue forward toward the wall.
  setMotors(NODE5_FWD_SPEED, NODE5_FWD_SPEED);
  node5BypassDone = true;
}
...
void setup() {
  ...
  // ===== SHARP IR SETUP (ADDED) =====
  pinMode(SHARP1_PIN, INPUT);
  pinMode(SHARP2_PIN, INPUT);
...
void loop() {
  ...
  // ===== OBSTACLE SENSORS (ULTRASONIC + 2x SHARP) =====
  // Sample all three sensors every ~60ms (keeps loop responsive).
  if (millis() - lastUSms >= US_PERIOD_MS) {
    lastUSms = millis();
    usCm     = readUltrasonicCmFast();
    sharp1Cm = readSharpCmFast(SHARP1_PIN);
    sharp2Cm = readSharpCmFast(SHARP2_PIN);
  }

  // Use the nearest *valid* detection across all sensors.
  int detectCm = minPositive3(usCm, sharp1Cm, sharp2Cm);
...
// Don't interfere while sitting on a marker bar
  ...
  if (!onBar) {
    ...
    // ===== NODE 5 SPUR SPECIAL CASE =====
    // Once we leave MJ35 toward M5, line following is disabled (after 2s) and we
    // drive by dead-reckoning until the wall. If an obstacle is encountered on this spur,
    // we must DRIVE AROUND it (not block the segment).
    if (headingTo5Spur && detectCm > 0) {
      // On the MJ35->M5 spur there is *no line* and we define node 5 by a wall.
      // USER RULE: the FIRST detection is always the obstacle, the SECOND detection is the wall.
      // So we count detections with a cooldown + re-arm gap.

      // Re-arm once we're "clear" again and cooldown passed.
      if (!node5DetectArmed) {
        if ((int32_t)(millis() - node5DetectCooldownUntil) >= 0 && detectCm >= NODE5_CLEAR_DETECT_CM) {
          node5DetectArmed = true;
        }
      }

      // Count a detection.
      if (node5DetectArmed && detectCm > 0 && detectCm <= NODE5_DETECT_CM) {
        node5DetectArmed = false;
        node5DetectCooldownUntil = millis() + NODE5_DETECT_COOLDOWN_MS;
        node5DetectCount++;
        Serial.print("NODE5 spur detection #"); Serial.print(node5DetectCount);
        Serial.print(" at "); Serial.print(detectCm); Serial.println(" cm");

        if (node5DetectCount == 1) {
          // First hit = obstacle: execute bypass.
          doNode5BypassManeuver();
          delay(5);
          return;
        } else {
          // Second hit (or later) = wall: arrive.
          arriveAtWallNode5();
          return;
        }
      }

      // Otherwise keep driving toward wall (line-follow may already be disabled).
      if (!lineFollowEnabled) {
        setMotors(NODE5_FWD_SPEED, NODE5_FWD_SPEED);
        delay(5);
        return;
      } else {
        // first 2 seconds: still following line as we exit MJ35, keep normal followLine()
        // below (do NOT return here)
      }
    }

    // If we detect obstacle elsewhere, trigger the return routine.
    // NEVER do this on the node-5 spur; spur obstacles are handled above.
    if (!headingTo5Spur && !returningToPrev && detectCm > 0 && detectCm <= STOP_CM) {
      setMotors(0, 0);
      delay(60);
...
if (obstOverrideActive && (millis() - obstOverrideArmedAt) > OBST_OVERRIDE_TTL_MS) {
  ...
  obstOverrideActive = false;
    ...
    if (curr == MJ35 && next == M5) {
      ...
      // Enter node-5 spur mode
      headingTo5Spur = true;
      node5BypassDone = false;
      node5SpurStartMs = millis();

      node5DetectCount = 0;
      node5DetectArmed = true;
      node5DetectCooldownUntil = 0;
    }
...
if (obstOverrideActive && (millis() - obstOverrideArmedAt) > OBST_OVERRIDE_TTL_MS) {
  ...
  obstOverrideActive = false;
    ...
    if (curr == MJ35 && next == M5) {
      ...
      // Enter node-5 spur mode (NORMAL MODE)
      headingTo5Spur = true;
      node5BypassDone = false;
      node5SpurStartMs = millis();
      node5DetectCount = 0;
...
void arriveAtWallNode5() {
  ...
  // Exit node-5 spur mode
  headingTo5Spur = false;
  node5BypassDone = false;
...
// ===============================
// ===== SHARP IR HELPERS (ADDED) =====
// ===============================
// Converts a Sharp analog output (ADC) to an approximate distance in cm.
// This is an *approximation*; you should calibrate the constants for your exact Sharp model.
// Returns -1 if reading looks invalid/out of range.
int readSharpCmFast(int adcPin) {
  // ESP32 ADC is noisy; take a tiny average of a few fast reads.
  const int N = 10;
  uint32_t acc = 0;
  for (int i = 0; i < N; i++) {
    acc += (uint32_t)analogRead(adcPin);
    delayMicroseconds(150);
  }
  float raw = (float)acc / (float)N;

  // Convert to voltage (assume 12-bit ADC and ~3.3V reference).
  float v = (raw / 4095.0f) * 3.3f;
  if (v < 0.05f) return -1;

  // Empirical inverse-power model (common for GP2Y0Axx family).
  // Tuned to be "reasonable" in the 4–30 cm region.
  float cm = 12.08f * powf(v, -1.058f);

  if (cm < 1.0f || cm > 200.0f) return -1;
  return (int)(cm + 0.5f);
}

// Returns the smallest positive value among a,b,c, or -1 if none are positive.
int minPositive3(int a, int b, int c) {
  int m = -1;
  if (a > 0) m = a;
  if (b > 0 && (m < 0 || b < m)) m = b;
